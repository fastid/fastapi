...
from fastid import password_hasher, typing
...
async def test_hasher_password_hasher_memory_profile_low():
    with patch('fastid.settings.settings.password_hasher_memory_profile', new_callable=PropertyMock) as mock_foo:
        ...
        password_string = await password_hasher.hasher(password=typing.Password('test'))
...
async def test_verify():
    password_hash = await password_hasher.hasher(password=typing.Password('test'))
    assert await password_hasher.verify(password_hash=password_hash, password=typing.Password('test'))


async def test_verify_fail():
    with pytest.raises(InternalServerException):
        await password_hasher.verify(password_hash='1234', password=typing.Password('test'))
